...
func BenchmarkDriftSlip(b *testing.B) {
	sc := NewSlipController(DriftCorrectionSlip, 2, 2)
	frameCount := 1024
	frameSize := 4
	buf := make([]byte, frameCount*frameSize)
	drift := mockDrift{ppm: 50.0}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = sc.ProcessFrame(buf, frameCount, drift)
	}
}

func FuzzDriftSlip(f *testing.F) {
	f.Add(float64(50.0), 1024)
	f.Add(float64(-50.0), 1024)
	f.Add(float64(100000.0), 10)
	f.Fuzz(func(t *testing.T, ppm float64, frameCount int) {
